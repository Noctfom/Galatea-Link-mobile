                    ...
                    try {
                        ...
                        val newTensor =
                            when {
                                ...
                                // Int32 to Int16 with explicit saturation for Galatea V3 narrow tensors
                                dataType == "int32" && targetType == "int16" -> {
                                    val intBuffer = tensor.intBuffer
                                    val intArray = IntArray(intBuffer.remaining())
                                    intBuffer.get(intArray)

                                    val shortArray = ShortArray(intArray.size) {
                                        intArray[it].coerceIn(Short.MIN_VALUE.toInt(), Short.MAX_VALUE.toInt()).toShort()
                                    }
                                    OnnxTensor.createTensor(
                                        ortEnvironment,
                                        ShortBuffer.wrap(shortArray),
                                        shape,
                                        OnnxJavaType.INT16,
                                    )
                                }
                                // Int32 to Int8 with explicit saturation for Galatea V3 narrow tensors
                                dataType == "int32" && targetType == "int8" -> {
                                    val intBuffer = tensor.intBuffer
                                    val intArray = IntArray(intBuffer.remaining())
                                    intBuffer.get(intArray)

                                    val byteArray = ByteArray(intArray.size) {
                                        intArray[it].coerceIn(Byte.MIN_VALUE.toInt(), Byte.MAX_VALUE.toInt()).toByte()
                                    }
                                    OnnxTensor.createTensor(
                                        ortEnvironment,
